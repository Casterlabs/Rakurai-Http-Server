...
public interface HttpStatus {
    ...
    default String statusString() {
        String str = String.valueOf(this.statusCode());
    ...
    public static @Nullable HttpStatus lookup(int code) {
        try {
            return StandardHttpStatus.STATUS_BY_CODE[code];
     ...
     */
    @NonNull
    public static enum StandardHttpStatus implements HttpStatus {
        // @formatter:off
...
//                                             UNUSED(306, "Unused");
        ...
        private static final StandardHttpStatus[] STATUS_BY_CODE;
        static {
            int maxCode = 0;
            for (StandardHttpStatus status : StandardHttpStatus.values()) {
                if (status.statusCode > maxCode) {
                    maxCode = status.statusCode;
                }
            }

            STATUS_BY_CODE = new StandardHttpStatus[maxCode + 1];
            for (StandardHttpStatus status : StandardHttpStatus.values()) {
